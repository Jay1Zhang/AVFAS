...
class AVVP_Evaluator(object):
    def __init__(self):
        super(AVVP_Evaluator, self).__init__()
    ...
    def stats_segment_f1(self):
        f1_a = 100 * np.mean(np.array(self.F_seg_a))
        f1_v = 100 * np.mean(np.array(self.F_seg_v))
        f1_av = 100 * np.mean(np.array(self.F_seg_av))
        f1_avg_type = (f1_a + f1_v + f1_av) / 3.
        f1_avg_event = 100 * np.mean(np.array(self.F_seg))

        return f1_a, f1_v, f1_av, f1_avg_type, f1_avg_event

    def stats_event_f1(self):
        f1_a = 100 * np.mean(np.array(self.F_event_a))
        f1_v = 100 * np.mean(np.array(self.F_event_v))
        f1_av = 100 * np.mean(np.array(self.F_event_av))
        f1_avg_type = (f1_a + f1_v + f1_av) / 3.
        f1_avg_event = 100 * np.mean(np.array(self.F_event))

        return f1_a, f1_v, f1_av, f1_avg_type, f1_avg_event


    def calc_f1(self, SO_a, SO_v, SO_av, GT_a, GT_v, GT_av):
        # segment-level F1 scores
        f_a, f_v, f, f_av = segment_level(SO_a, SO_v, SO_av, GT_a, GT_v, GT_av)
        self.F_seg_a.append(f_a)
        self.F_seg_v.append(f_v)
        self.F_seg_av.append(f_av)
        self.F_seg.append(f)

        # event-level F1 scores
        f_a, f_v, f, f_av = event_level(SO_a, SO_v, SO_av, GT_a, GT_v, GT_av)
        self.F_event_a.append(f_a)
        self.F_event_v.append(f_v)
        self.F_event_av.append(f_av)
        self.F_event.append(f)
...
def event_wise_metric(event_p, event_gt):
    ...
    if event_p is not None:
        ...
        for i in range(num_event):
            x1 = event_p[i]             # one-hot
            if event_gt is not None:
                nn = len(event_gt)
                flag = True
                for j in range(nn):
                    x2 = event_gt[j]    # one-hot
...
if __name__ == '__main__':
    seg = np.random.randint(0, 1, size=(10))
    n = 1
    print(seg)
    extract_event(seg, n)
